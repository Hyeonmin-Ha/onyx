 ...
 */
public final class NonSerializedElement<T> implements Block<T> {
  private final int key;
  private final T nonSerializedData;

  /**
   * Creates a non-serialized element having a specific key value.
   *
   * @param key  the key.
   * @param data the non-serialized data.
   */
  public NonSerializedElement(final int key,
                              final T data) {
    this.key = key;
